...
create table dbo.RecipeIngredient (
    ...
    Amount decimal (4,2) not null 
        constraint c_RecipeIngredient_Amount_must_be_greater_than_0 check(Amount > 0),
    SequenceOfIngredients int 
        constraint c_RecipeIngredient_SequenceOfIngredient_must_be_greater_than_0 check(SequenceOfIngredients > 0),
...
--YF I would make the varchar limit larger, just in case its a long step. 100 char is not wrong, but not a lot 
        constraint c_Directions_cannot_be_blank check(Direction <> ''),
    SequenceOfDirections int 
        constraint c_Directions_SequenceOfDirections_must_be_greater_than_0 check(SequenceOfDirections > 0),
...
create table dbo.Meal (
    ...
    DateMealCreated datetime not null default getdate()
        constraint c_Meal_DateMealCreated_cannot_be_blank check(DateMealCreated <> ''),
...
create table dbo.MealCourseRecipes (
    ...
    MealCourseId int not null constraint f_MealCourse_MealCourseRecipes foreign key references MealCourse(MealCourseId),
    RecipeId int not null constraint f_Recipe_MealCourseRecipes foreign key references Recipe(RecipeId),
...
create table dbo.CookBooks (
    ...
    UsersId int not null constraint f_Users_CookBooks foreign key references Users(UsersId),
    CookBookName varchar(100) not null 
        constraint c_CookBooks_CookBookName_cannot_be_blank check(CookBookName <> '')
        constraint u_CookBooks_CookBookName unique,
    Price decimal(5,2) not null 
        constraint c_CookBooks_Price_must_be_greater_than_0 check(Price > 0),
    CookBookActive bit not null default 0,
    DateBookCreated date not null default getdate()
        constraint c_DateBookCreated_cannot_be_blank check(DateBookCreated <> ''),
